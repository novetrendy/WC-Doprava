...
function doprava_dpd_zobrazit_pobocky() {
  if ( is_ajax() ) {
    ...
    if ( strpos( $chosen_shipping_method[0], "doprava_dpd" ) !== false ) { ?>
      <tr class="dpd">
        <th>
          <img src="' . plugin_basename( dirname( __FILE__ ) ) . 'WC-Doprava-main/loga/DPD_logo_redgrad_rgb-300x133.png" width="200" border="0">
        </th>
        <td>
  ...
  if ( is_checkout() ) {
    $dpd_settings = get_option( 'woocommerce_doprava_dpd_settings' ); 
     $api_klic = $dpd_settings['op1']; ?>
      <script src="' . plugin_basename( dirname( __FILE__ ) ) . 'WC-Doprava-main/js/dpd.js"></script>
...
function doprava_posta_zobrazit_pobocky() {
  if ( is_ajax() ) {
    ...
    if ( strpos( $chosen_shipping_method[0], "doprava_posta" ) !== false ) { ?>
      <tr class="posta">
        <th>
          <img src="' . plugin_basename( dirname( __FILE__ ) ) . 'WC-Doprava-main/loga/cp.svg" width="200" border="0">
        </th>
        <td>
        <input type="hidden" id="packeta-point-id" name="packeta-point-id" value="<?php echo $dpd_branches; ?>">
  ...
  if ( is_checkout() ) {
    $posta_settings = get_option( 'woocommerce_doprava_posta_settings' ); 
     ...
     $api_klic2 = $posta_settings['op2']; ?>
      <script src="' . plugin_basename( dirname( __FILE__ ) ) . 'WC-Doprava-main/js/cp.js"></script>
      <script type="text/javascript">
        var packetaApiKey = '<?php echo $api_klic; ?>';
...
function doprava_wedo_zobrazit_pobocky() {
  if ( is_ajax() ) {
    ...
    if ( strpos( $chosen_shipping_method[0], "doprava_wedo" ) !== false ) { ?>
      <tr class="wedo">
        <th>
          <img src="' . plugin_basename( dirname( __FILE__ ) ) . 'WC-Doprava-main/loga/WE_DO_na_bile_RGB.png" width="200" border="0">
        </th>
        <td>
            <input type="hidden" id="packeta-point-id" name="packeta-point-id" value="<?php echo $dpd_branches; ?>">
  ...
  if ( is_checkout() ) {
    $wedo_settings = get_option( 'woocommerce_doprava_wedo_settings' ); 
     ...
     $api_klic2 = $wedo_settings['op2']; ?>
      <script src="' . plugin_basename( dirname( __FILE__ ) ) . 'WC-Doprava-main/js/wedo.js"></script>
      <script type="text/javascript">
        var packetaApiKey = '<?php echo $api_klic; ?>';
        var packetaApiKey2= '<?php echo $api_klic2; ?>';
        var $storage_support = true;
...
function doprava_gls_zobrazit_pobocky() {
  if ( is_ajax() ) {
    ...
    if ( strpos( $chosen_shipping_method[0], "doprava_gls" ) !== false ) { ?>
      <tr class="gls">
        <th>
          <img src="' . plugin_basename( dirname( __FILE__ ) ) . 'WC-Doprava-main/loga/GLS_Logo_2021_RGB_GLSBlue.png" width="200" border="0">
        </th>
        <td>
         <input type="hidden" id="packeta-point-id" name="packeta-point-id" value="<?php echo $dpd_branches; ?>">
  ...
  if ( is_checkout() ) {
    $gls_settings = get_option( 'woocommerce_doprava_gls_settings' ); 
     ...
     $api_klic2 = $gls_settings['op2']; ?>
      <script src="' . plugin_basename( dirname( __FILE__ ) ) . 'WC-Doprava-main/js/gls.js"></script>
      <script type="text/javascript">
        var packetaApiKey = '<?php echo $api_klic; ?>';
        var packetaApiKey2= '<?php echo $api_klic2; ?>';
        var $storage_support = true;
